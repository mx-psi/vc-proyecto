...
En esta sección describimos el algoritmo iterativo de estimación de homografías 2D a partir de un conjunto de correspondencias haciendo uso del error de Sampson. Hemos decidido implementar el algoritmo de optimización manualmente para hacer nosotros mismos todo el proceso.
...
Entrada
:  Como parámetros necesarios, una $f$ que queremos minimizar, un punto `inicial` desde el que empezamos a explorar y el `objetivo` al que queremos llegar con $f$ (0, en nuestro caso). Los parámetros opciones son el número máximo de iteraciones en `max_iter` y el umbral cerca del objetivo en el que paramos la búsqueda, en `umbral`.

Salida
:  Un punto $x$ que minimiza la expresión $|f(x)-\mathtt{objetivo}|$ y el error, que es justo el valor de $|f(x)-\mathtt{objetivo}|$.
...
2. Mientras `norm` > `umbral` y `iters` < `max_iter`:
...
Para el análisis de los resultados hemos decidido aplicar el cálculo de las homografías con error de Sampson al problema de creación de mosaicos con el que trabajamos en la práctica 2, de tal forma que podamos observar visualmente la calidad de los resultados. Como solo queríamos ver una pequeña muestra de cómo actúa el algoritmo, y de ver que de verdad daba una homografía aceptable, hemos usado las imágenes de yosemite que se nos daban de muestra para las prácticas. Hemos cogido un par de ellas, y cogiendo una lista de correspondencias entre puntos (obtenida de las prácticas anteriores) hemos probado el algoritmo.
...
\begin{figure}[h!]
\begin{center}
  \includegraphics[width=0.8\textwidth]{Implementacion/resultados/outliers1}

  \includegraphics[width=0.8\textwidth]{Implementacion/resultados/outliers2}
...
\begin{figure}[h!]
\begin{center}
  \includegraphics[width=0.8\textwidth]{Implementacion/resultados/bueno1}
...
En caso de que queramos ejecutar el programa manualmente con dos imágenes llamamos al algoritmo con:
```bash
